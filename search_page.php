...
<section class="products_cont">
    <div class="pro_box_cont">
        ...
        if (isset($_POST['submit'])) {
            ...
            $selected_genre = isset($_POST['genre']) ? mysqli_real_escape_string($conn, $_POST['genre']) : "";

            // Base query
            $query = "SELECT * FROM `products` WHERE 1";

            // Add filters only if values are given
            if (!empty($search_item)) {
                $query .= " AND name LIKE '%$search_item%'";
            }
            ...
            if (mysqli_num_rows($select_products) > 0) {
                while ($fetch_products = mysqli_fetch_assoc($select_products)) {
                    ...
                    <form action="" method="post" class="pro_box">
                        ...
                        <p>Genre: <?php echo $fetch_products['genre']; ?></p>
